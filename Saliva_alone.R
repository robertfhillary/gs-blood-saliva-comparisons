...
linker1 = linker[linker$source %in% "saliva",]
...
# 2 samples having 1 % of sites with a detection p-value greater than 0.05
# Samples removed: # 203253040183_R05C01, 203253040111_R04C01
#2782 sites were removed as beadcount <3 in 5 % of samples
#16078 sites having 1 % of samples with a detection p-value greater than 0.05 were removed

keep_samps <- sampleNames(MSet.pf) # 2 samples removed
...
write.table(Table3, file="Normalisation_performancesalive.txt", sep='\t', row.names=F)

# Save normalied dataset corresponding to ranked means - danet used here. 
saveRDS(bndanet, file="Outputs/Normalised_Beta_Values_saliva_danet.rds")
